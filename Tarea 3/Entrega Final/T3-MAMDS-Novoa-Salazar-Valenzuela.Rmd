...
### Cargamos los datos de entrenamiento y tomamos en cuenta las variables numéricas para poder realizar un preprocesamiento y luego utilizarlos para los modelos que serán empleados posteriormente, a la vez eliminamos las variables "id" y "flight_number" ya que no aportan información relevante para el modelo.
...
### Transformamos la variable noshow a una variable categórica, con el supuesto de que valor mayor a 3 es 1 y si no toma el valor 0, esto para seguir con lo propuesto por el enunciado, es decir, desde 4 en adelante es un no show.
...
### Los datos de entrenamiento tienen una proporción de 1 es a 2 por lo tanto, están desbalanceados.

### Ahora analizaremos las correlaciones, para esto utilizaremos la función corr y luego realizamos un sampleo de 50.000 valores para poder realizar el análisis.
...
### Dividimos la data de entrenamiento
```{r}
...
### Creamos una función para eliminar los outliers siguiendo la regla de los cuartiles, es decir, el límite superior es 1.5 veces el rango intercuartil por sobre el tercer cuartil, mientras que el límite inferior es 1.5 veces el rango intercuartil por debajo del primer cuartil.
...
### Generamos el modelo de Support Vector Machine
...
### Generamos un modelo de ensamble, en este caso es Random Forest
...
### Para realizar el modelo stacked, utilizaremos tres modelos base: SVM, Naive Bayes y Árbol de Decisión
...
### Naive Bayes para el modelo stacked
...
### Árbol de Decisión para el modelo stacked
...
## El mejor modelo, desde el punto de vista del F1 Score, es el de Stacking ya que obtiene un puntaje de 0.5065

### Ahora procedemos a realizar la predicción para el conjunto de datos de evaluación, donde realizamos el mismo preprocesamiento que en el conjunto de datos de entrenamiento.

```{r}
eval <- read.csv("ALUMNOS-evalData.csv")
...
### Por último con la predicción generamos el archivo csv con 248.880 estimaciones realizadas para el modelo.
...
write.csv(x_est2, "pred-evalData.csv", row.names = FALSE)
```
